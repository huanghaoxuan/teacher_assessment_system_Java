...
@RestController
@RequestMapping("/scoreGovernmentachievementaward")
@Api(tags = "【得分】政府部门设立的教学（科研）成果奖的数据接口")
public class ScoreGovernmentachievementawardController {

    @Autowired
    private ScoreGovernmentachievementawardService scoreGovernmentachievementawardService;

    @RequestMapping(value = "/selectByPrimaryKey", method = {RequestMethod.GET})
    @ApiOperation(value = "查询政府部门设立的教学（科研）成果奖", notes = "政府部门设立的教学（科研）成果奖查询接口")
    public ScoreGovernmentachievementaward selectByPrimaryKey() {
        return scoreGovernmentachievementawardService.selectByPrimaryKey(1);
    }


    @RequestMapping(value = "/updateByPrimaryKey", method = {RequestMethod.POST})
    @ApiOperation(value = "更新政府部门设立的教学（科研）成果奖", notes = "更新学生比赛（毕业设计）获奖查询接口")
    @ApiImplicitParams({
    ...
    public int updateByPrimaryKey(@ApiIgnore BigDecimal countryFirstDigit, BigDecimal countrySecondDigit, BigDecimal countryThirdDigit, BigDecimal provincialFirstDigit, BigDecimal provincialSecondDigit, BigDecimal provincialThirdDigit, BigDecimal cityFirstDigit, BigDecimal citySecondDigit, BigDecimal cityThirdDigit, BigDecimal schoolFirstDigit, BigDecimal schoolSecondDigit, BigDecimal schoolThirdDigit) {
        ...
        ScoreGovernmentachievementaward scoreGovernmentachievementaward = new ScoreGovernmentachievementaward(1, countryFirstDigit, countrySecondDigit, countryThirdDigit, provincialFirstDigit, provincialSecondDigit, provincialThirdDigit, cityFirstDigit, citySecondDigit, cityThirdDigit, schoolFirstDigit, schoolSecondDigit, schoolThirdDigit);
        return scoreGovernmentachievementawardService.updateByPrimaryKey(scoreGovernmentachievementaward);
    }
}
